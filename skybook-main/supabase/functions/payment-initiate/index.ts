...
const envSuffixForBrand=(brandCode:unknown)=>safeText(brandCode).toUpperCase().replace(/[^A-Z0-9]+/g,'_').replace(/^_+|_+$/g,'')

const getBrandEnv=(baseKey:string,brandCode:unknown)=>{
  const suffix=envSuffixForBrand(brandCode)
  const baseWithoutPrefix=baseKey.replace(/^DPO_/,'')
  const candidates=suffix ? [`${baseKey}_${suffix}`,`DPO_${suffix}_${baseWithoutPrefix}`,`${suffix}_${baseKey}`,baseKey] : [baseKey]
...
const requireBrandEnv=(baseKey:string,brandCode:unknown,message:string)=>{
  const value=getBrandEnv(baseKey,brandCode)
  if(!value)throw new Error(message)
  return value
}
...
class DpoProvider implements PaymentProvider{
  async initiatePayment(payload:PaymentProviderPayload,context:PaymentContext){
    const brandCode=safeText(context.booking.brand_code) || 'true-travel'
    const companyToken=requireBrandEnv('DPO_COMPANY_TOKEN',brandCode,'DPO_COMPANY_TOKEN is missing. Add your DPO merchant company token before enabling DPO checkout.')
    const serviceType=requireBrandEnv('DPO_SERVICE_TYPE',brandCode,'DPO_SERVICE_TYPE is missing. Add your DPO service type before enabling DPO checkout.')
    const apiUrl=getBrandEnv('DPO_API_URL',brandCode) || 'https://secure.3gdirectpay.com/API/v6/'
    const paymentTimeLimit=getBrandEnv('DPO_PAYMENT_TIME_LIMIT_HOURS',brandCode) || '24'
