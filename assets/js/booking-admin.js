...
const openDocAsPdf=(title,fullHtml,options={})=>{
  ...
  const gen='<script src="'+SB_PDF_LIB_URL+'"></script><script>(function(){var CB=parent["'+cb+'"];function run(){try{var o={margin:[6,6,8,6],image:{type:"jpeg",quality:0.98},html2canvas:{scale:2,backgroundColor:"#ffffff",useCORS:true,logging:false},jsPDF:{unit:"mm",format:"a4",orientation:"portrait"},pagebreak:{mode:["css","legacy"]}};html2pdf().set(o).from(document.body).toPdf().get("pdf").then(function(p){try{var innerH=283,innerW=198;var ratio=innerW/document.body.scrollWidth;var hMm=document.body.scrollHeight*ratio;var pages=p.internal.getNumberOfPages();var lastContent=hMm-(pages-1)*innerH;if(pages>1&&lastContent<8)p.deletePage(pages)}catch(e){}CB&&CB(p.output("blob"))}).catch(function(){CB&&CB(null)})}catch(e){CB&&CB(null)}}window.addEventListener("load",function(){if(typeof html2pdf!=="undefined")return run();var f=document.createElement("script");f.src="'+SB_PDF_LIB_FALLBACK_URL+'";f.onload=run;f.onerror=function(){CB&&CB(null)};document.body.appendChild(f)})})()</script>'
...
const SB_WORKBENCH_PDF_CSS='.sb-report-cards{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;margin-top:16px}.metric-card{padding:14px 16px;border:1px solid #dbe8f4;border-left:4px solid #1976d2;border-radius:12px;background:linear-gradient(180deg,#f7fbff,#eef5fd);box-shadow:none;page-break-inside:avoid}.metric-card span{display:block;font-size:10px;text-transform:uppercase;letter-spacing:.07em;color:#5f7383}.metric-card strong{display:block;margin-top:7px;font-size:20px;font-weight:800;color:#0f2b52;letter-spacing:-.01em}.report-split-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px;page-break-inside:avoid}.report-split-grid article{border:1px solid #dbe8f4;border-radius:12px;padding:16px;background:#fff;min-width:0;box-shadow:0 2px 6px rgba(15,79,168,.05);page-break-inside:avoid}.report-split-grid h4{margin:0 0 12px;color:#0f4fa8;font-size:12px;text-transform:uppercase;letter-spacing:.05em;border-bottom:2px solid #e1ecf6;padding-bottom:8px}.report-stat-list{display:grid;gap:8px;margin-top:8px}.report-stat-list div{padding:10px 12px;border:1px solid #dbe8f4;border-radius:10px;background:#f7fbff}.report-stat-list strong{display:block}.report-stat-list span{display:block;margin-top:4px;color:#5f7383;font-size:12px}.status-badge{display:inline-block;padding:3px 10px;border-radius:999px;background:#e8f4ff;color:#1e5b93;font-size:11px;font-weight:700}.status-badge.is-bad{background:#fdecec;color:#a33a3a}.muted-copy,.field-hint{color:#5f6f80;font-size:12px}.table-subline{font-size:11px;color:#5f6f80}.empty-state strong{display:block;font-size:12px}.empty-state span{font-size:12px;color:#5f6f80}.admin-spacer{margin-top:18px}table{table-layout:fixed}th,td{overflow-wrap:break-word;word-break:break-word;font-size:11px;padding:8px 9px;vertical-align:top}thead th{font-size:9px;letter-spacing:.02em;white-space:normal;word-break:normal;overflow-wrap:normal;padding:8px 6px}td:first-child{overflow-wrap:normal;word-break:normal}.sb-report-foot{margin-top:16px;padding-top:10px;border-top:1px solid #e1ecf6;text-align:center;font-size:10px;letter-spacing:.08em;text-transform:uppercase;color:#8299ad}body{padding:24px 30px 16px}header{margin-bottom:4px;padding:20px 24px}section{margin-top:16px;padding-top:14px}'
...
const downloadWorkbenchReportPdf=key=>{
  ...
  downloadSkyBookReportPdf(title,`
    ...
    <div class="sb-report-foot">SkyBook — Tour Operations &amp; Booking Admin</div>
  `,filename)
}
